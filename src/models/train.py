...
#     # cb_params
#     cb_params = model_params['CatBoost']
#     logger.info("cat boost parameters read")
    
#     # build catboost model
#     cb = CatBoostRegressor(**cb_params)
#     logger.info("built catboost model")
...
#     stacking_reg = StackingRegressor(estimators=[("cb_model",cb),
#                                                  ("lgbm_model",lgbm)],
#                                      final_estimator=lr,
#                                      cv=5,n_jobs=-1)
...
def save_pickle(obj, save_dir: Path, filename: str):
    save_location = save_dir / filename
...
if __name__ == "__main__":
    ...
    stacking_reg = StackingRegressor(
        estimators=[("cb_model", cb), ("lgbm_model", lgbm)],
        final_estimator=lr,
        cv=5,
        n_jobs=-1
    )
    ...
    logger.info("Wrapped model saved")

    # save stacking model
    save_pickle(model.regressor_, model_save_dir, "stacking_regressor.pkl")
    logger.info("Stacking regressor saved")

    # save transformer
    save_pickle(model.transformer_, model_save_dir, "power_transformer.pkl")
    logger.info("Power transformer saved")
